...
import { CollisionResult, CollisionResultSuccess, compareObjects, computeClosestPoint, computeIntersectRect, createEventOptions } from '../utils'
			...
			if (collisionDetection === DndCollision.ClosestPoint) {
				...
				if (!collision.success || collision.distance > outsideThreshold) {
					...
					if (debug) {
						...
						line.setAttribute('x2', collision.element.rect.center.x + collision.pointOfContact?.x)
						line.setAttribute('y1', ev.pageY.toString())
						line.setAttribute('y2', collision.element.rect.center.y + collision.pointOfContact.y)
					}
