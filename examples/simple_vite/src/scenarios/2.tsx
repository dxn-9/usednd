        ...
        <div className="flex w-screen items-center justify-center flex-col">
                ...
                onDrop={(ev) => {
                    ...
                    if (o_isFolder && a_isFolder) {
                        ...
                        setFolderState((prev) => {
                            ...
                            // to review
                            copy[a_folder].subfolders = [
                                ...copy[a_folder].subfolders!.slice(0, a_subfolder),
                                ...copy[a_folder].subfolders!.slice(a_subfolder + 1),
                            ]
                            copy[o_folder].subfolders = copy[o_folder]
                                .subfolders!.slice(0, targetIndex)
                                .concat([activeFolder])
                                .concat(copy[o_folder].subfolders!.slice(targetIndex))
            ...
            >
                {folderState.map((folder, index) => (
                    <ParentFolder key={folder.id} folder={folder} index={index} />
                ))}
            </DndProvider>
        </div>
    )
...
const ParentFolder = ({ children, folder, index }: PropsWithChildren<{ folder: Folder; index: number }>) => {
    ...
    return (
        <div
            className={`w-64 bg-orange-400   flex flex-col border border-black p-1 ${overClasses}`}
            ref={setNode}
            {...listeners}
        ...
        >
            ...
            <div className="flex flex-col gap-2">
                {folder.subfolders?.map((subfolder, subIndex) => (
                    <Subfolder key={subfolder.id} folder={subfolder} parentIndex={index} index={subIndex} />
                ))}
            </div>
        </div>
    )
        ...
        <>
            <div
                ...
                style={{
                    ...
                    background: over.isOver ? 'green' : '',
                }}
            >
                <p>
                    {' '}
                    SUB : {folder.name} - {folder.id} - {parentIndex}-{index}{' '}
                </p>
            </div>
        </>
    )
